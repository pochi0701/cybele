...
void global_param_init(void)
{
    ...
    // cbl.conf 変更対応 allow_user_agent
    strncpy(global_param.system_password,"", sizeof(global_param.system_password) );
    return;
}
// ****************************************************
// True,Falseの設定
    ...
    while ( 1 ){
        ...
        if ( strlen(line_buf) > 0 ) // 値が入ってた。
        {
            ...
            if (( strlen(key) > 0 ) && (strlen(value) > 0 ))
            {
                ...
                // alias
                else if ( _stricmp("alias", key) == 0 && count_alias < MAX_COUNT_ALIAS){
                    // valueを' 'で分割
                ...
                // flag_use_skin
                else if ( _stricmp("flag_use_skin", key) == 0 ){
                    readTF(value,global_param.flag_use_skin);
                }
                // skin_root
                else if ( _stricmp("skin_root", key) == 0 ){
                    #ifdef linux
                ...
                // skin_name
                else if ( _stricmp("skin_name", key) == 0 ){
                ...
                // access_allow
                    ...
                    strncpy(global_param.system_password, value , sizeof(global_param.system_password));
                }

#ifndef linux
                // work_root
                else if ( _stricmp("work_root", key) == 0 ){
                    strncpy(global_param.work_root, value, sizeof(global_param.work_root));
                    //                                        printf("global_param.skin_name '%s' open.\n", global_param.skin_name);
                    // 最後が'/'じゃなかったら、'/'を追加
                    if ( global_param.work_root[strlen(global_param.work_root)-1 ] != DELIMITER[0] ){
                        strncat(global_param.work_root, DELIMITER, sizeof(global_param.work_root));
                        // printf("global_param.skin_name_B '%s' open.\n", global_param.skin_name);
                    }
                }
#else
                // user_agent_proxy_override
                else if ( _stricmp("user_agent_proxy_override", key) == 0 ){
                    strncpy(global_param.user_agent_proxy_override, value , sizeof(global_param.user_agent_proxy_override));
                }
                // flag_execute_cgi
                else if ( _stricmp("flag_execute_cgi", key) == 0 ){
                    readTF(value,global_param.flag_execute_cgi);
                }
                // flag_allow_proxy
                else if ( _stricmp("flag_allow_proxy", key) == 0 ){
                    readTF(value,global_param.flag_allow_proxy);
                }
                // access_allow
                else if ( _stricmp("access_allow", key) == 0 ){
...
void check_file_extension_to_mime_type(const char *file_extension, char *mime_type, int mime_type_size )
{
    ...
    for (i=0;;i++){
        ...
        break;
        if ( _stricmp(mime_list[i].file_extension, file_extension) == 0 ){
